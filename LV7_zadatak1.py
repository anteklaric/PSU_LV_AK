...
print(f"\nTočnost nad podacima iz trening skupa: {train_acc:.2f}")
print(f"Točnost nad podacima iz testnog skupa: {test_acc:.2f}")
...
sns.heatmap(matrica_zabune, annot=True, fmt="d", cmap="Reds")
...
for i, idx in enumerate(incorrect_indices[:10]):
    ...
    plt.title(f"Predvidjeno: {y_pred[idx]}\nStvarno: {y_test[idx]}")
    plt.axis('off')
plt.tight_layout()
plt.show()
